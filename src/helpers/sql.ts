...
const escapeSingleQuotes = (str: string) => str.replace(/'/gm, `''`)

const formatHex = (hex) => {
  // use 256 bit by default
...
export class SQLEncoder {
  ...
  constructor(flavor: 'postgres' | 'sqlite') {
    this.flavor = flavor
  }

  parseDoc(table: SchemaTable, doc: any) {
    for (const row of table.rows) {
      if (row.type === 'bigint' && doc[row.name]) {
        const data = doc[row.name].data ?? doc[row.name]
        Object.assign(doc, {
          [row.name]: BigInt('0x'+formatHex(Buffer.from(data).toString('hex')))
        })
      }
    }
    return doc
  ...
  parseType(type: string, value: any) {
    ...
    if (type === 'bigint' && typeof value === 'bigint') {
      // store as a blob
      if (this.flavor === 'postgres') {
        return `'\\x${formatHex(value.toString(16))}'`
      } else if (this.flavor === 'sqlite') {
        return `x'${formatHex(value.toString(16))}'`
      }
    }
  ...
  whereToSql(table: SchemaTable, doc: any = {}, sqlOnly = false) {
    ...
    const sql = Object.keys(doc)
      .map(key => {
        ...
        if (Array.isArray(val)) {
          ...
          const values = val.map(v => this.parseType(rowDef.type, v))
        ...
        if (typeof val === 'object' && val !== null) {
          ...
          return Object.keys(val).map(k => {
            if (k === 'nin') {
              ...
              const values = val[k].map((v: any) => this.parseType(rowDef.type, v))
            ...
            const parsed = this.parseType(rowDef.type, val[k])
            return `"${key}" ${operator} ${parsed}`
          })
        }
        const parsed = this.parseType(rowDef.type, val)
    ...
    if (Array.isArray(doc.OR)) {
      orConditions =
        doc.OR.length > 0
          ? doc.OR.map((w: any) => this.whereToSql(table, w, true)).join(' OR ')
          : 'false'
    }
    const andConditions =
      Array.isArray(doc.AND) && doc.AND.length > 0
        ? doc.AND.map((w: any) => this.whereToSql(table, w, true)).join(' AND ')
        : 'true'
    return ` ${sqlOnly ? '' : 'WHERE'} (${sql ||
      'true'}) AND (${orConditions}) AND (${andConditions})`
  }

  tableCreationSql(tableData: TableData[], postgres = false) {
    ...
    for (const doc of docs) {
      const values = keys
        .map(k => {
          ...
          return this.parseType(rowDef.type, doc[k])
        })
        .join(',')
  ...
  findManySql(
    ...
    return `SELECT * FROM "${table.name}" ${this.whereToSql(
      table,
      where,
    )} ${orderBy} ${limit} ${offset};`
  }

  countSql(table: SchemaTable, where: WhereClause): string {
    return `SELECT COUNT(*) FROM "${table.name}" ${this.whereToSql(table, where)};`
  }

  updateSql(table: SchemaTable, options: UpdateOptions): string {
    ...
    const setSql = Object.keys(update)
      .map(key => {
        ...
        if (!rowDef)
          throw new Error(`Unable to find row definition for key: "${key}"`)
        return `"${key}" = ${this.parseType(rowDef.type, update[key])}`
      })
      .join(', ')
    return `UPDATE "${table.name}" SET ${setSql} ${this.whereToSql(table, where)};`
  }
  ...
  deleteManySql(
    ...
    return `DELETE FROM "${table.name}" ${this.whereToSql(table, options.where)};`
    // return `DELETE FROM "${table.name}" WHERE "${constraintKey}" IN
    // (SELECT "${constraintKey}" FROM "${table.name}" ${this.whereToSql(
    //   table,
    //   options.where,
    // )} ${orderBySql} ${limitSql});`
  }

  upsertSql(table: SchemaTable, options: UpsertOptions): string {
    const { sql } = this.createSql(table, options.create)
    ...
    const updateSqlCommand = Object.keys(options.update)
      .map(key => {
        ...
        if (!rowDef)
          throw new Error(`Unable to find row definition for key: "${key}"`)
        return `"${key}" = ${this.parseType(rowDef.type, options.update[key])}`
      })
      .join(', ')
