...
export class BlockCache {
  ...
  constructor(web3: Web3, db: DB) {
    ...
    this.blockHeaderSubscription = this.web3.eth
      .subscribe('newBlockHeaders', err => {
        if (err) {
          console.log(
            `block-cache - Error subscribing to block headers: ${err.toString()}`,
          )
        }
      })
      .on('data', async blockHeader => {
        ...
        try {
          ...
          console.log(
            `block-cache - Error writing block cache changes: ${err.toString()}`,
          )
        }
      })
  }
  ...
  async writeChangesIfNeeded() {
    ...
    for (const op of this.pendingOperations) {
      ...
      // otherwise write
      try {
        console.log(`Writing ${op.collection}`)
